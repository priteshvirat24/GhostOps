...
def test_eval_test_a_golden_dataset_loads():
    ...
    assert GoldenDatasetRegistry.DATASET_VERSION == "ghostops-golden-v2"
    assert GoldenDatasetRegistry.CORPUS_VERSION == "ghostops-history-v1"
...
def test_eval_test_b_c_d_hybrid_retrieval_and_precision_metrics(db_session: Session):
    ...
    res = AgentEvaluationHarness.run_benchmark(db_session, split="development")
...
def test_eval_test_j_k_evaluation_persisted_and_immutable(db_session: Session):
    ...
    assert run_in_db.total_cases == 10
...
def test_eval_test_m_api_endpoints(client):
    ...
    run_resp = client.post("/api/v1/evaluation/benchmark?split=development")
    ...
    assert len(run_detail["cases"]) == 10
